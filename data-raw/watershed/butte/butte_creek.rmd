...
**Data Source:** [FWS 2003](s3://cvpiahabitat-r-package/Butte_Creek_Spring-run_chinook_salmon_spawning_8-29-2003.pdf){target="_blank"} (77-78) and [FERC relicensing of DeSabla](https://s3-us-west-2.amazonaws.com/cvpiahabitat-r-package/cvpia-sit-model-inputs/DeSabla2008ButteIFIM.pdf){target="_blank"} (APPENDIX E6.3.2.6-J1) 
...
rear <- read_excel('data-raw/mark_gard_data/IFIMWUA.xlsx', 
  ...
  rename(flow_cfs = Flow, FR_fry_wua = `Fry Rearing`, FR_juv_wua = `Juv Rearing`, ST_adult_wua = `adult trout`)
...
butte_creek_instream <- sr_spawn %>% 
  ...
  select(flow_cfs, FR_fry_wua, FR_juv_wua, SR_spawn_wua, ST_spawn_wua, ST_adult_wua, watershed)
...
butte_creek_instream %>% 
  ...
  select(flow_cfs, FR_fry_wua, FR_juv_wua, ST_adult_wua, watershed) %>% 
  head(5) %>% 
...
butte_creek_instream %>% 
    ...
    theme_minimal() +
    scale_color_manual(labels = c('Steelhead', 'Fall and Spring Run'), 
...
``` {r}
labs <- c('Fall Run - fry', 'Fall Run - juvenile', 'Steelhead - adult')
names(labs) <- c('FR_fry_wua', 'FR_juv_wua', 'ST_adult_wua')

butte_creek_instream %>% 
  select(flow_cfs, FR_fry_wua, FR_juv_wua, ST_adult_wua) %>% 

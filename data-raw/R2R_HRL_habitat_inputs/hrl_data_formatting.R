...
all_habitat_data_for_hrl_inputs_all_runs <- read_csv("data-raw/R2R_HRL_habitat_inputs/hrl_commitments_by_watershed.csv") |> 
  ...
  mutate(run = case_when(run == "fall + spring" ~ "Fall and Spring Run",
                         run == "all" ~ "Fall and Winter and Spring Run",
                         run == "unknown" ~ "Unknown Run",
                         TRUE ~ NA)) |> 

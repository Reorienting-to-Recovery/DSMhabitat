...
salmonid_extents <- st_read("data/rearing_spatial_data/salmonid_habitat_extents/salmonid_habitat_extents.shp", 
...
baseline_projects <- readRDS('data/map_data/baseline_projects.RDS')
...
hqt <- readRDS('data/map_data/hqt.RDS')

tmh_reaches <- readRDS('data/map_data/TMH_reaches.rds') |> 
  st_transform(crs = 4326) |> 
  group_by(river) |> 
  mutate(id = cur_group_id())

rim_dams <- readRDS('data/map_data/rim_dams.RDS')
dam_barriers <- readRDS('data/map_data/dam_barriers.RDS')
...
leaflet() %>% 
      ...
      addPolygons(data = hqt, group = "Valley Lowland (<0.04%)", popup = "Habitat Quantification Tool Boundary", color = "darkgrey",opacity = .5,

...
DSMhabitat::apply_suitability(
  DSMhabitat::square_meters_to_acres(
    DSMhabitat::set_floodplain_habitat(watershed = 'Feather River', species = 'fr', flow = 5000)))

# FEATHER R NR GRIDLEY CA-------------------
feather <- dataRetrieval::readNWISdv(siteNumbers = '11407150', parameterCd = '00060',
                                  startDate = '1980-01-01', endDate = '1997-12-31')

fp_threshold_flow <- DSMhabitat::feather_river_floodplain$flow_cfs[which(cumsum(DSMhabitat::feather_river_floodplain$FR_floodplain_acres != 0) == 1)]
...
days_inundated <- feather %>%
  ...
  mutate(fp_area_acres = DSMhabitat::apply_suitability(
    DSMhabitat::square_meters_to_acres(
      DSMhabitat::set_floodplain_habitat(watershed = 'Feather River', species = 'fr', flow = monthly_mean_flow))))
...
data.frame(
  ...
  weeks_inundated = 0:4,
  flow_threshhold = c(0, NA, NA, NA, 4000)
) %>% write_rds('data-raw/floodplain_inundation_thresholds/feather_river_inundated.rds')

...
add_project_habitat <- DSMhabitat::fr_spawn$lto_12a["American River" , , ] * 
  hab_prop_change_from_projects("spawning", "American River", "fr", "adult", "LTO_12a")
updated_habitat <- DSMhabitat::fr_spawn$lto_12a["American River", , ] + add_project_habitat

r_to_r_lto_12a_baseline_fr_spawn["American River", , ] <- updated_habitat 

# add clear creek spawning habitat
add_project_habitat <- DSMhabitat::fr_spawn$lto_12a["Clear Creek" , , ] * 
  hab_prop_change_from_projects("spawning", "Clear Creek", "fr", "adult", "LTO_12a")
updated_habitat <- DSMhabitat::fr_spawn$lto_12a["Clear Creek", , ] + add_project_habitat

r_to_r_lto_12a_baseline_fr_spawn["Clear Creek", , ] <- updated_habitat 

# add paynes creek spawning habitat
add_project_habitat <- DSMhabitat::fr_spawn$lto_12a["Paynes Creek" , , ] * 
  hab_prop_change_from_projects("spawning", "Paynes Creek", "fr", "adult", "LTO_12a")
updated_habitat <- DSMhabitat::fr_spawn$lto_12a["Paynes Creek", , ] + add_project_habitat

r_to_r_lto_12a_baseline_fr_spawn["Paynes Creek", , ] <- updated_habitat 

# Add sacramento river spawning habitat 
add_project_habitat <- DSMhabitat::fr_spawn$lto_12a["Upper Sacramento River" , , ] * 
  hab_prop_change_from_projects("spawning", "Upper Sacramento River", "fr", "adult", "LTO_12a")
updated_habitat <- DSMhabitat::fr_spawn$lto_12a["Upper Sacramento River", , ] + add_project_habitat
...
# Exploratory plot 
r_to_r_baseline_lto_12a <- r_to_r_lto_12a_baseline_fr_spawn |> 
...
spawn <- expand_grid(
  ...
  mutate(
    ...
    r_to_r_baseline_LTO_12a = as.vector(r_to_r_baseline_lto_12a)) |> 
...
spawn |> 
  transmute(watershed, date = ymd(paste(year, month, 1)), 
            r_to_r_baseline, r_to_r_baseline_LTO_12a) |> 
...
r_to_r_lto_12a_baseline_fr_juv <- DSMhabitat::fr_juv$lto_12a
r_to_r_lto_12a_baseline_fr_fry <- DSMhabitat::fr_fry$lto_12a

# Add american river juv habitat 
add_project_habitat <- DSMhabitat::fr_juv$lto_12a["American River" , , ] * 
  hab_prop_change_from_projects("inchannel rearing", "American River", "fr", "juv", "LTO_12a")
updated_habitat <- DSMhabitat::fr_juv$lto_12a["American River", , ] + add_project_habitat

r_to_r_lto_12a_baseline_fr_juv["American River", , ] <- updated_habitat 

# Add american river fry habitat 
add_project_habitat <- DSMhabitat::fr_fry$lto_12a["American River" , , ] * 
  hab_prop_change_from_projects("inchannel rearing", "American River", "fr", "fry", "LTO_12a")
updated_habitat <- DSMhabitat::fr_fry$lto_12a["American River", , ] + add_project_habitat

r_to_r_lto_12a_baseline_fr_fry["American River", , ] <- updated_habitat 

# add tuolumne river juv habitat
add_project_habitat <- DSMhabitat::fr_juv$lto_12a["Tuolumne River" , , ] * 
  hab_prop_change_from_projects("inchannel rearing", "Tuolumne River", "fr", "juv", "LTO_12a")
updated_habitat <- DSMhabitat::fr_juv$lto_12a["Tuolumne River", , ] + add_project_habitat

r_to_r_lto_12a_baseline_fr_juv["Tuolumne River", , ] <- updated_habitat 

# add tuolumne river fry habitat
add_project_habitat <- DSMhabitat::fr_fry$lto_12a["Tuolumne River" , , ] * 
  hab_prop_change_from_projects("inchannel rearing", "Tuolumne River", "fr", "fry", "LTO_12a")
updated_habitat <- DSMhabitat::fr_fry$lto_12a["Tuolumne River", , ] + add_project_habitat

r_to_r_lto_12a_baseline_fr_fry["Tuolumne River", , ] <- updated_habitat 

# Upper sac river 
# Add upper sac river juv habitat 
add_project_habitat <- DSMhabitat::fr_juv$lto_12a["Upper Sacramento River" , , ] * 
  hab_prop_change_from_projects("inchannel rearing", "Upper Sacramento River", "fr", "juv", "LTO_12a")
updated_habitat <- DSMhabitat::fr_juv$lto_12a["Upper Sacramento River", , ] + add_project_habitat

r_to_r_lto_12a_baseline_fr_juv["Upper Sacramento River", , ] <- updated_habitat 

# Add upper sacramento  river fry habitat 
add_project_habitat <- DSMhabitat::fr_fry$lto_12a["Upper Sacramento River" , , ] * 
  hab_prop_change_from_projects("inchannel rearing", "Upper Sacramento River", "fr", "fry", "LTO_12a")

updated_habitat <- DSMhabitat::fr_fry$lto_12a["Upper Sacramento River", , ] + add_project_habitat
...
add_project_habitat <- DSMhabitat::fr_juv$lto_12a["Upper-mid Sacramento River" , , ] * 
  hab_prop_change_from_projects("inchannel rearing", "Upper-mid Sacramento River", 
                                "fr", "juv", "LTO_12a")
updated_habitat <- DSMhabitat::fr_juv$lto_12a["Upper-mid Sacramento River", , ] + add_project_habitat

r_to_r_lto_12a_baseline_fr_juv["Upper-mid Sacramento River", , ] <- updated_habitat 

# Add upper mid sacramento river fry habitat 
add_project_habitat <- DSMhabitat::fr_fry$lto_12a["Upper-mid Sacramento River" , , ] * 
  hab_prop_change_from_projects("inchannel rearing", "Upper-mid Sacramento River", 
                                "fr", "fry", "LTO_12a")
updated_habitat <- DSMhabitat::fr_fry$lto_12a["Upper-mid Sacramento River", , ] + add_project_habitat

r_to_r_lto_12a_baseline_fr_fry["Upper-mid Sacramento River", , ] <- updated_habitat 
...
# Exploratory plot 
r_to_r_baseline_lto_12a <- r_to_r_lto_12a_baseline_fr_juv |> 
  DSMhabitat::square_meters_to_acres()
...
# Exploratory plot 
r_to_r_baseline_lto_12a <- r_to_r_lto_12a_baseline_fr_fry |> 
  DSMhabitat::square_meters_to_acres()
...
add_project_habitat <- DSMhabitat::fr_fp$lto_12a["Lower-mid Sacramento River" , , ] * 
  hab_prop_change_from_projects("floodplain rearing", "Lower-mid Sacramento River" , 
                                "fr", "juv", "LTO_12a")
updated_habitat <- DSMhabitat::fr_fp$lto_12a["Lower-mid Sacramento River" , , ] + add_project_habitat

r_to_r_lto_12a_baseline_fr_fp["Lower-mid Sacramento River" , , ] <- updated_habitat 

# TUolumne river floodplain 
add_project_habitat <- DSMhabitat::fr_fp$lto_12a["Tuolumne River" , , ] * 
  hab_prop_change_from_projects("floodplain rearing", "Tuolumne River" , 
                                "fr", "juv", "LTO_12a")
updated_habitat <- DSMhabitat::fr_fp$lto_12a["Tuolumne River" , , ] + add_project_habitat

r_to_r_lto_12a_baseline_fr_fp["Tuolumne River" , , ] <- updated_habitat 

# Add Yuba floodlplain habitat 
add_project_habitat <- DSMhabitat::fr_fp$lto_12a["Yuba River" , , ] * 
  hab_prop_change_from_projects("floodplain rearing", "Yuba River" , 
                                "fr", "juv", "LTO_12a")
updated_habitat <- DSMhabitat::fr_fp$lto_12a["Yuba River" , , ] + add_project_habitat
...
# Exploratory plot 
r_to_r_baseline_lto_12a <- r_to_r_lto_12a_baseline_fr_fp |> 
  DSMhabitat::square_meters_to_acres()
...
# Exploratory plot 
r_to_r_baseline_lto_12a <- r_to_r_lto_12a_baseline_sr_spawn |> 
  DSMhabitat::square_meters_to_acres()
...
# Add inchannel habitat to both fry and juvenile habitat objects ---------------
# set r_to_r_baseline_fr_juv and fry 
r_to_r_lto_12a_baseline_sr_juv <- DSMhabitat::sr_juv$lto_12a
r_to_r_lto_12a_baseline_sr_fry <- DSMhabitat::sr_fry$lto_12a

# add tuolumne river juv habitat
add_project_habitat <- DSMhabitat::sr_juv$lto_12a["Tuolumne River" , , ] * 
  hab_prop_change_from_projects("inchannel rearing", "Tuolumne River", "sr", "juv", "LTO_12a")
updated_habitat <- DSMhabitat::sr_juv$lto_12a["Tuolumne River", , ] + add_project_habitat

r_to_r_lto_12a_baseline_sr_juv["Tuolumne River", , ] <- updated_habitat 

# add tuolumne river fry habitat
add_project_habitat <- DSMhabitat::sr_fry$lto_12a["Tuolumne River" , , ] * 
  hab_prop_change_from_projects("inchannel rearing", "Tuolumne River", "sr", "fry", "LTO_12a")
updated_habitat <- DSMhabitat::sr_fry$lto_12a["Tuolumne River", , ] + add_project_habitat

r_to_r_lto_12a_baseline_sr_fry["Tuolumne River", , ] <- updated_habitat 

# Upper sac river 
# Add upper sac river juv habitat 
add_project_habitat <- DSMhabitat::sr_juv$lto_12a["Upper Sacramento River" , , ] * 
  hab_prop_change_from_projects("inchannel rearing", "Upper Sacramento River", "sr", "juv", "LTO_12a")
updated_habitat <- DSMhabitat::sr_juv$lto_12a["Upper Sacramento River", , ] + add_project_habitat

r_to_r_lto_12a_baseline_sr_juv["Upper Sacramento River", , ] <- updated_habitat 

# Add upper sacramento  river fry habitat 
add_project_habitat <- DSMhabitat::sr_fry$lto_12a["Upper Sacramento River" , , ] * 
  hab_prop_change_from_projects("inchannel rearing", "Upper Sacramento River", "sr", "fry", "LTO_12a")

updated_habitat <- DSMhabitat::sr_fry$lto_12a["Upper Sacramento River", , ] + add_project_habitat

r_to_r_lto_12a_baseline_sr_fry["Upper Sacramento River", , ] <- updated_habitat 

# add Upper-mid Sacramento River
# Add upper mid sac river juv habitat 
add_project_habitat <- DSMhabitat::sr_juv$lto_12a["Upper-mid Sacramento River" , , ] * 
  hab_prop_change_from_projects("inchannel rearing", "Upper-mid Sacramento River", 
                                "sr", "juv", "LTO_12a")
updated_habitat <- DSMhabitat::sr_juv$lto_12a["Upper-mid Sacramento River", , ] + add_project_habitat

r_to_r_lto_12a_baseline_sr_juv["Upper-mid Sacramento River", , ] <- updated_habitat 

# Add upper mid sacramento river fry habitat 
add_project_habitat <- DSMhabitat::sr_fry$lto_12a["Upper-mid Sacramento River" , , ] * 
  hab_prop_change_from_projects("inchannel rearing", "Upper-mid Sacramento River", 
                                "sr", "fry", "LTO_12a")
updated_habitat <- DSMhabitat::sr_fry$lto_12a["Upper-mid Sacramento River", , ] + add_project_habitat

r_to_r_lto_12a_baseline_sr_fry["Upper-mid Sacramento River", , ] <- updated_habitat 
...
# Exploratory plot 
r_to_r_baseline_lto_12a <- r_to_r_lto_12a_baseline_sr_juv |> 
  DSMhabitat::square_meters_to_acres()
...
# Add Lower-mid sacramento river floodplain habitat 
add_project_habitat <- DSMhabitat::sr_fp$lto_12a["Lower-mid Sacramento River" , , ] * 
  hab_prop_change_from_projects("floodplain rearing", "Lower-mid Sacramento River" , 
                                "sr", "juv", "LTO_12a")
updated_habitat <- DSMhabitat::sr_fp$lto_12a["Lower-mid Sacramento River" , , ] + add_project_habitat

r_to_r_lto_12a_baseline_sr_fp["Lower-mid Sacramento River" , , ] <- updated_habitat 

# Tuolumne river floodplain 
add_project_habitat <- DSMhabitat::sr_fp$lto_12a["Tuolumne River" , , ] * 
  hab_prop_change_from_projects("floodplain rearing", "Tuolumne River" , 
                                "sr", "juv", "LTO_12a")
updated_habitat <- DSMhabitat::sr_fp$lto_12a["Tuolumne River" , , ] + add_project_habitat

r_to_r_lto_12a_baseline_sr_fp["Tuolumne River" , , ] <- updated_habitat 

# Add Yuba floodlplain habitat 
add_project_habitat <- DSMhabitat::sr_fp$lto_12a["Yuba River" , , ] * 
  hab_prop_change_from_projects("floodplain rearing", "Yuba River" , 
                                "sr", "juv", "LTO_12a")
updated_habitat <- DSMhabitat::sr_fp$lto_12a["Yuba River" , , ] + add_project_habitat
...
# Add sacramento river spawning habitat 
add_project_habitat <- DSMhabitat::wr_spawn$lto_12a["Upper Sacramento River" , , ] * 
  hab_prop_change_from_projects("spawning", "Upper Sacramento River", "wr", "adult", "LTO_12a")
updated_habitat <- DSMhabitat::wr_spawn$lto_12a["Upper Sacramento River", , ] + add_project_habitat
...
# Add inchannel habitat to both fry and juvenile habitat objects ---------------
# set r_to_r_baseline_fr_juv and fry 
r_to_r_lto_12a_baseline_wr_juv <- DSMhabitat::wr_juv$lto_12a
r_to_r_lto_12a_baseline_wr_fry <- DSMhabitat::wr_fry$lto_12a

# Upper sac river 
# Add upper sac river juv habitat 
add_project_habitat <- DSMhabitat::wr_juv$lto_12a["Upper Sacramento River" , , ] * 
  hab_prop_change_from_projects("inchannel rearing", "Upper Sacramento River", "wr", "juv", "LTO_12a")
updated_habitat <- DSMhabitat::wr_juv$lto_12a["Upper Sacramento River", , ] + add_project_habitat

r_to_r_lto_12a_baseline_wr_juv["Upper Sacramento River", , ] <- updated_habitat 

# Add upper sacramento  river fry habitat 
add_project_habitat <- DSMhabitat::wr_fry$lto_12a["Upper Sacramento River" , , ] * 
  hab_prop_change_from_projects("inchannel rearing", "Upper Sacramento River", "wr", "fry", "LTO_12a")

updated_habitat <- DSMhabitat::wr_fry$lto_12a["Upper Sacramento River", , ] + add_project_habitat

r_to_r_lto_12a_baseline_wr_fry["Upper Sacramento River", , ] <- updated_habitat 

# add Upper-mid Sacramento River
# Add upper mid sac river juv habitat 
add_project_habitat <- DSMhabitat::wr_juv$lto_12a["Upper-mid Sacramento River" , , ] * 
  hab_prop_change_from_projects("inchannel rearing", "Upper-mid Sacramento River", 
                                "wr", "juv", "LTO_12a")
updated_habitat <- DSMhabitat::wr_juv$lto_12a["Upper-mid Sacramento River", , ] + add_project_habitat

r_to_r_lto_12a_baseline_wr_juv["Upper-mid Sacramento River", , ] <- updated_habitat 

# Add upper mid sacramento river fry habitat 
add_project_habitat <- DSMhabitat::wr_fry$lto_12a["Upper-mid Sacramento River" , , ] * 
  hab_prop_change_from_projects("inchannel rearing", "Upper-mid Sacramento River", 
                                "wr", "fry", "LTO_12a")
updated_habitat <- DSMhabitat::wr_fry$lto_12a["Upper-mid Sacramento River", , ] + add_project_habitat

r_to_r_lto_12a_baseline_wr_fry["Upper-mid Sacramento River", , ] <- updated_habitat 
...
# Add Lower-mid sacramento river floodplain habitat 
add_project_habitat <- DSMhabitat::wr_fp$lto_12a["Lower-mid Sacramento River" , , ] * 
  hab_prop_change_from_projects("floodplain rearing", "Lower-mid Sacramento River" , 
                                "wr", "juv", "LTO_12a")
updated_habitat <- DSMhabitat::wr_fp$lto_12a["Lower-mid Sacramento River" , , ] + add_project_habitat

r_to_r_lto_12a_baseline_wr_fp["Lower-mid Sacramento River" , , ] <- updated_habitat 
...
r_to_r_baseline_delta_lto_12a <- DSMhabitat::delta_habitat$sit_habitat
...
r_to_r_baseline_delta_lto_12a[ , , "North Delta"] <- updated_habitat 
...
delta <- expand_grid(
  ...
  mutate(
    ...
    baseline_habitat_lto_12a = as.vector(baseline_habitat_lto_12a)) |> 
  filter(watershed %in% c("North Delta"))

delta |> 
  transmute(watershed, date = ymd(paste(year, month, 1)), 
            baseline, baseline_habitat_lto_12a) |> 
  gather(version, acres, -watershed, -date)  |> 
  ggplot(aes(date, acres, color = version)) +
  geom_line() + 
  # geom_col(position = 'dodge') +

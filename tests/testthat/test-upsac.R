...
test_that("modeling of species coverage hasn't changed - Upper Sac", {
  modeling <- subset(DSMhabitat::modeling_exist, Watershed == 'Upper Sacramento River')
...
test_that('FR juv Upper Sac River works', {
  ...
  flow1 <- DSMhabitat::upper_sacramento_river_instream$flow_cfs[1]

  expect_equal(
    set_instream_habitat(watershed = 'Upper Sacramento River',
                         species = 'fr', life_stage = 'juv',
                         flow = flow1),
    DSMhabitat::upper_sacramento_river_instream$rearing_sq_meters[1])

})

test_that('FR spawn Upper Sac works', {

  spawn_not_na_index1 <- which(!is.na(DSMhabitat::upper_sac_ACID_boards_in$FR_spawn_WUA))[1]
  spawn_not_na_index2 <- which(!is.na(DSMhabitat::upper_sac_ACID_boards_out$FR_spawn_WUA))[1]

  spawn_wua1 <- DSMhabitat::upper_sac_ACID_boards_in$FR_spawn_WUA[spawn_not_na_index1]
  spawn_wua2 <- DSMhabitat::upper_sac_ACID_boards_out$FR_spawn_WUA[spawn_not_na_index2]
  ...
  spawn_flow1 <- DSMhabitat::upper_sac_ACID_boards_in$flow_cfs[spawn_not_na_index1]
  spawn_flow2 <- DSMhabitat::upper_sac_ACID_boards_out$flow_cfs[spawn_not_na_index2]
...
test_that('ST spawn Upper Sac works', {
  ...
  flow1 <- DSMhabitat::upper_sac_ACID_boards_in$flow_cfs[1]
  flow2 <- DSMhabitat::upper_sac_ACID_boards_out$flow_cfs[1]
...
test_that('FR floodplain Upper Sac River works', {
  first_flood_index <-  which(DSMhabitat::upper_sacramento_river_floodplain$floodplain_sq_meters > 0)[1]

  flow <- DSMhabitat::upper_sacramento_river_floodplain$flow_cfs[first_flood_index]
  floodplain <- subset(DSMhabitat::upper_sacramento_river_floodplain,flow_cfs == flow)$floodplain_sq_meters

...
test_that("modeling of species coverage hasn't changed - Feather", {
  modeling <- subset(DSMhabitat::modeling_exist, Watershed == 'Feather River')
...
test_that('FR instream Feather River works', {
  ...
  fry_not_na_index <- which(!is.na(DSMhabitat::feather_river_instream$FR_fry_wua))[1]
  juv_not_na_index <- which(!is.na(DSMhabitat::feather_river_instream$FR_juv_wua))[1]
  spawn_not_na_index <- which(!is.na(DSMhabitat::feather_river_instream$FR_spawn_wua))[1]

  fry_wua <- DSMhabitat::feather_river_instream$FR_fry_wua[fry_not_na_index]
  juv_wua <- DSMhabitat::feather_river_instream$FR_juv_wua[juv_not_na_index]
  spawn_wua <- DSMhabitat::feather_river_instream$FR_spawn_wua[spawn_not_na_index]
  ...
  fry_flow <- DSMhabitat::feather_river_instream$flow_cfs[fry_not_na_index]
  juv_flow <- DSMhabitat::feather_river_instream$flow_cfs[juv_not_na_index]
  spawn_flow <- DSMhabitat::feather_river_instream$flow_cfs[spawn_not_na_index]
...
test_that('ST spawn Feather River works', {
  ...
  spawn_not_na_index <- which(!is.na(DSMhabitat::feather_river_instream$ST_spawn_wua))[1]
  spawn_wua <- DSMhabitat::feather_river_instream$ST_spawn_wua[spawn_not_na_index]
...
test_that('FR floodplain Feather River works', {
  first_flood_index <-  which(DSMhabitat::feather_river_floodplain$FR_floodplain_acres > 0)[1]

  flow <- DSMhabitat::feather_river_floodplain$flow_cfs[first_flood_index]
  floodplain <- subset(DSMhabitat::feather_river_floodplain,flow_cfs == flow)$FR_floodplain_acres
...
test_that('No WR or LFR on Feather River', {
  ...
  expect_true(is.na(set_floodplain_habitat('Feather River', 'lfr', 2000)))
})
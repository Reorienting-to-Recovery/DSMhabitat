...
## FLOODPLAIN:
### Fall run:
fr_fp <- modifyList(DSMhabitat::fr_fp, list(r_to_r_tmh = r_to_r_tmh_fr_flood)) 
usethis::use_data(fr_fp, overwrite = TRUE)
### Spring Run:
sr_fp <- modifyList(DSMhabitat::sr_fp, list(r_to_r_tmh = r_to_r_tmh_sr_flood))
usethis::use_data(sr_fp, overwrite = TRUE)
#### Winter Run:
wr_fp <- modifyList(DSMhabitat::wr_fp, list(r_to_r_tmh = r_to_r_tmh_wr_flood))
usethis::use_data(wr_fp, overwrite = TRUE)

## IN CHANNEL REARING:
### Fall Run: 
fr_fry <- modifyList(DSMhabitat::fr_fry, list(r_to_r_tmh = r_to_r_tmh_fr_fry))
usethis::use_data(fr_fry, overwrite = TRUE)

fr_juv <- modifyList(DSMhabitat::fr_juv, list(r_to_r_tmh = r_to_r_tmh_fr_juv))
usethis::use_data(fr_juv, overwrite = TRUE)
### Spring Run: 
sr_fry <- modifyList(DSMhabitat::sr_fry, list(r_to_r_tmh = r_to_r_tmh_sr_fry))
usethis::use_data(sr_fry, overwrite = TRUE)

sr_juv <- modifyList(DSMhabitat::sr_juv, list(r_to_r_tmh = r_to_r_tmh_sr_juv))
usethis::use_data(sr_juv, overwrite = TRUE)
### Winter Run: 
wr_fry <- modifyList(DSMhabitat::wr_fry, list(r_to_r_tmh = r_to_r_tmh_wr_fry))
usethis::use_data(wr_fry, overwrite = TRUE)

wr_juv <- modifyList(DSMhabitat::wr_juv, list(r_to_r_tmh = r_to_r_tmh_wr_juv))
usethis::use_data(wr_juv, overwrite = TRUE)

## SPAWNING: 
### Fall run: 
fr_spawn <- modifyList(DSMhabitat::fr_spawn, list(r_to_r_tmh = r_to_r_tmh_fr_spawn))
usethis::use_data(fr_spawn, overwrite = TRUE)
### Spring Run: 
sr_spawn <- modifyList(DSMhabitat::sr_spawn, list(r_to_r_tmh = r_to_r_tmh_sr_spawn))
usethis::use_data(sr_spawn, overwrite = TRUE)
### Winter Run:
wr_spawn <- modifyList(DSMhabitat::wr_spawn, list(r_to_r_tmh = r_to_r_tmh_wr_spawn))

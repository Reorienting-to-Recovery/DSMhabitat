...
DSMhabitat::apply_suitability(
  DSMhabitat::square_meters_to_acres(
    DSMhabitat::set_floodplain_habitat(watershed = 'Stanislaus River', species = 'fr', flow = 2000)))
...
fp_threshold_flow <- DSMhabitat::stanislaus_river_floodplain$flow_cfs[which(cumsum(DSMhabitat::stanislaus_river_floodplain$FR_floodplain_acres != 0) == 1)]
...
days_inundated <- stanislaus %>%
  ...
  mutate(fp_area_acres = DSMhabitat::apply_suitability(
    DSMhabitat::square_meters_to_acres(
      DSMhabitat::set_floodplain_habitat(watershed = 'Stanislaus River', species = 'fr', flow = monthly_mean_flow))))

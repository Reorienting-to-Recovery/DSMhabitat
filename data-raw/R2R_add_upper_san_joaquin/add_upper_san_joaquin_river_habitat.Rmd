...
all_reaches_sa_diff <- map_df(reaches, ~{
  ...
  add_order <- all_files_sa |> 
    ...
    select(-order) |> 
    arrange(max_order) |> 
    mutate(id = match(max_order, unique(max_order))) |> 
    select(-max_order) 
...
# exploratory plot of suitability criteria: 
# ggplot() +
#   geom_line(data = yuba_suit |> 
#               filter(units_si_conv < 100), aes(x = units_si_conv, y = suitability_index)) +
#   facet_wrap(~suitability_metric + units_conv + life_stage)

```

## Instream Rearing Habitat

The 1D Hec Ras model was run for for flows 100 through 4500 CFS at all nodes and reaches except 4B1. 4B1 was excluded because it is not currently part of migration corridor for salmonids. A suitability based on depth and velocity was applied all nodes and flows to get a suitable area for in-channel and floodplain rearing for the Upper San Joaquin River between Merced River and Friant Dam. 

Suitability criteria was applied to the outputs of the Hec Ras model for suitable depth and velocoty for instream juvenile and fry rearing. The Yuba River depth and velocity criteria was used as a proxy for the Upper San Joaquin River using data provided by Mark Gard from the California Department of Fish and Wildlife. 
...
fry_depth <- yuba_suit |> filter(life_stage == "Fry" &  suitability_metric == "Depth") |> 
  filter(suitability_index != 0 & units_si_conv > 0) |> 
...
fp_sa <- all_ras_results |> 
  mutate(total_fp_sa = sa_right_diff +  sa_left_diff) |> 
  group_by(reach, q_total) |> 
  summarise(sum_sa_area_fp = sum(total_fp_sa, na.rm = TRUE))
  
# TODO: apply a suitability? 90% like TMH? 
...
<!-- ## Save New Spring Run Data Objects  -->
...
hab_func <- approxfun(upper_san_joaquin_instream$flow_cfs, upper_san_joaquin_instream$SR_spawn_acres , rule = 2)
...
r_to_r_add_sj_sr_spawn <- DSMhabitat::sr_spawn$r_to_r_baseline
new_spawn_hab <- DSMhabitat::sr_spawn$r_to_r_baseline['San Joaquin River', , ] + hab['San Joaquin River', , ]
r_to_r_add_sj_sr_spawn["San Joaquin River" , , ] <- new_spawn_hab 

baseline_sr_spawn <- DSMhabitat::sr_spawn
baseline_sr_spawn$r_to_r_baseline <- r_to_r_add_sj_sr_spawn
sr_spawn <- baseline_sr_spawn

# sr_spawn$r_to_r_baseline == DSMhabitat::sr_spawn$r_to_r_baseline
...
hab_func <- approxfun(upper_san_joaquin_instream$flow_cfs, upper_san_joaquin_instream$SR_fry_acres , rule = 2)
habitat_area <- hab_func(flows)
...
# add habitats together 
r_to_r_add_sj_sr_fry <- DSMhabitat::sr_fry$r_to_r_baseline
new_fry_hab <- DSMhabitat::sr_fry$r_to_r_baseline['San Joaquin River', , ] + hab['San Joaquin River', , ]
r_to_r_add_sj_sr_fry["San Joaquin River" , , ] <- new_fry_hab 

baseline_sr_fry <- DSMhabitat::sr_fry
baseline_sr_fry$r_to_r_baseline <- r_to_r_add_sj_sr_fry
sr_fry <- baseline_sr_fry

# sr_fry$r_to_r_baseline == DSMhabitat::sr_fry$r_to_r_baseline

#usethis::use_data(sr_fry, overwrite = TRUE)

# JUVENILES: 
hab_func <- approxfun(upper_san_joaquin_instream$flow_cfs, upper_san_joaquin_instream$SR_juv_acres , rule = 2)
habitat_area <- hab_func(flows)
...
# add habitats together 
r_to_r_add_sj_sr_juv <- DSMhabitat::sr_juv$r_to_r_baseline
new_juv_hab <- DSMhabitat::sr_juv$r_to_r_baseline['San Joaquin River', , ] + hab['San Joaquin River', , ]
r_to_r_add_sj_sr_juv["San Joaquin River" , , ] <- new_juv_hab 

baseline_sr_juv <- DSMhabitat::sr_juv
baseline_sr_juv$r_to_r_baseline <- r_to_r_add_sj_sr_juv
sr_juv <- baseline_sr_juv

# sr_juv$r_to_r_baseline == DSMhabitat::sr_juv$r_to_r_baseline
...
hab_func <- approxfun(upper_san_joaquin_floodplain$flow_cfs, upper_san_joaquin_floodplain$SR_floodplain_acres , rule = 2)
habitat_area <- hab_func(flows)
...
# add habitats together 
r_to_r_add_sj_sr_fp <- DSMhabitat::sr_fp$r_to_r_baseline
new_fp_hab <- DSMhabitat::sr_fp$r_to_r_baseline['San Joaquin River', , ] + hab['San Joaquin River', , ]
r_to_r_add_sj_sr_fp["San Joaquin River" , , ] <- new_fp_hab 

# check to make sure it works 
# r_to_r_add_sj_fr_fp['San Joaquin River' , , ] == DSMhabitat::fr_fp$r_to_r_baseline['San Joaquin River' , , ]

baseline_sr_fp <- DSMhabitat::sr_fp
baseline_sr_fp$r_to_r_baseline <- r_to_r_add_sj_sr_fp
sr_fp <- baseline_sr_fp

# sr_fp$r_to_r_baseline == DSMhabitat::sr_fp$r_to_r_baseline

...
## Spawning Habitat 
```{r}
new <- DSMhabitat::fr_spawn$r_to_r_baseline %>% DSMhabitat::square_meters_to_acres()
...
## Fry Habitat 
```{r}
new <- DSMhabitat::fr_fry$r_to_r_baseline %>% DSMhabitat::square_meters_to_acres()
old <- DSMhabitat::fr_fry$biop_itp_2018_2019 %>% DSMhabitat::square_meters_to_acres()
...
## Juvenile Habitat
```{r}
new <- DSMhabitat::fr_juv$r_to_r_baseline %>% DSMhabitat::square_meters_to_acres()
old <- DSMhabitat::fr_juv$biop_itp_2018_2019 %>% DSMhabitat::square_meters_to_acres()
dimnames(old) <- dimnames(new)
juv <- expand_grid(
  watershed = factor(DSMscenario::watershed_labels, 
                     levels = DSMscenario::watershed_labels),
...
## Floodplain Habitat 
```{r}
new <- DSMhabitat::fr_fp$r_to_r_baseline %>% DSMhabitat::square_meters_to_acres()
old <- DSMhabitat::fr_fp$biop_itp_2018_2019 %>% DSMhabitat::square_meters_to_acres()
dimnames(old) <- dimnames(new)
fp <- expand_grid(
  watershed = factor(DSMscenario::watershed_labels, 
                     levels = DSMscenario::watershed_labels),

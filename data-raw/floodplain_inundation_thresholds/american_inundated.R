...
DSMhabitat::apply_suitability(
  DSMhabitat::square_meters_to_acres(
    DSMhabitat::set_floodplain_habitat(watershed = 'American River', species = 'fr', flow = 5000)))
...
fp_threshold_flow <- DSMhabitat::american_river_floodplain$flow_cfs[which(cumsum(DSMhabitat::american_river_floodplain$FR_floodplain_acres != 0) == 1) - 1]
...
days_inundated <- american %>%
  ...
  mutate(fp_area_acres = DSMhabitat::apply_suitability(
    DSMhabitat::square_meters_to_acres(
      DSMhabitat::set_floodplain_habitat(watershed = 'American River', species = 'fr', flow = monthly_mean_flow))))

...
DSMhabitat::apply_suitability(
  DSMhabitat::square_meters_to_acres(
    DSMhabitat::set_floodplain_habitat(watershed = 'Merced River', species = 'fr', flow = 2000)))

# MERCED R NR STEVINSON CA-------------------
merced <- dataRetrieval::readNWISdv(siteNumbers = '11272500', parameterCd = '00060',
                                    startDate = '1975-01-01', endDate = '1994-12-31')

fp_threshold_flow <- DSMhabitat::merced_river_floodplain$flow_cfs[which(cumsum(DSMhabitat::merced_river_floodplain$FR_floodplain_acres != 0) == 1)]
...
days_inundated <- merced %>%
  ...
  mutate(fp_area_acres = DSMhabitat::apply_suitability(
    DSMhabitat::square_meters_to_acres(
      DSMhabitat::set_floodplain_habitat(watershed = 'Merced River', species = 'fr', flow = monthly_mean_flow))))
...
data.frame(
  watershed = rep(c('Merced River'), 5),
  weeks_inundated = 0:4,
  flow_threshhold = c(0, NA, NA, NA, 500)
) %>% write_rds('data-raw/floodplain_inundation_thresholds/merced_river_inundated.rds')

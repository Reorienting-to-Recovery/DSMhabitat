...
The following plot shows suitable habitat in square meters for Fall Run and 
Late-Fall Run Chinook. Spring Run spawning was calculated using the a regional 
approximation approach, more information on this [here](#).

```{r}
deer_creek_instream %>% 
  select(flow_cfs, FR_spawn_hsi, LFR_spawn_hsi) %>% 
...
```{r}
deer_creek_instream %>% 
  select(flow_cfs, FR_juv_hsi, LFR_juv_hsi, ST_juv_hsi, SR_juv_hsi) %>% 
  gather(species, suitable_habitat, -flow_cfs) %>% 
  mutate(
    species = case_when(
      species == "FR_juv_hsi" ~ "Fall Run Chinook",
      species == "LFR_juv_hsi" ~ "Late-Fall Run Chinook",
      species == "SR_juv_hsi" ~ "Spring Run Chinook",
      species == "ST_juv_hsi" ~ "Steelhead",
      TRUE ~ NA_character_
    )
  ) %>% 
  ggplot(aes(flow_cfs, suitable_habitat/4046.86, color = species)) + 
...
deer_creek_floodplain %>% 
  ...
  rename(`Fall and Late-Fall Run Chinook` = FR_floodplain_acres, `Spring Run Chinook and Steelhead` = SR_floodplain_acres) %>% 

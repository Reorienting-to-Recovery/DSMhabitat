...
Curves shown above are functionalized and take `flow` as an input. The curves were run using `LTO_12a` flows as an input to generate new habitat. 
...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
# spawning
DSMhabitat::fr_juv$r_to_r_lto_12a_baseline["Tuolumne River",,] |> 
  as_tibble() |> 
  ...
  mutate(acres = DSMhabitat::square_meters_to_acres(sqm),
         date = as.Date(paste0("1970-", month, "-01"), format = "%Y-%b-%d")) |> 

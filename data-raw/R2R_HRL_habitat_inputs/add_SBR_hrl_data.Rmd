...
knitr::kable(head(american_hrl |> 
                    mutate(FR_juv_sqm_hrl = prettyNum(round(FR_juv_sqm_hrl), big.mark = ",")) |> select(Watershed = watershed, 
                                                                                                        `Flow (cfs)` = flow_cfs, 
                                                                                                        `VA habitat (square meters)` = FR_juv_sqm_hrl)))
```
...
existing_and_HRL |> 
  ...
  pivot_longer(c(existing, existing_and_HRL),
               names_to = "scenario",
               values_to = "value") |> 
  mutate(scenario = ifelse(scenario == "existing", "Existing", "Existing + HRL"),
...
bind_rows(
  ...
  mutate(va_enhanced_spawn_lfc, scenario = 'VA enhanced')) |> 
...
bind_rows(
  ...
  mutate(va_enhanced_rearing_lfc_fry, scenario = 'VA enhanced')) |> 
  ggplot(aes(flow_cfs, suitable_acres, color = scenario)) +
  geom_line() +
  theme_minimal() +
...
bind_rows(
  ...
  mutate(va_enhanced_rearing_lfc_juv, scenario = 'va enhanced')) |> 
  ggplot(aes(flow_cfs, suitable_acres, color = scenario)) +
  geom_line() +
  theme_minimal() +
...
bind_rows(
  ...
  mutate(va_enhanced_fp, scenario = 'VA enhanced')) |> 
  ggplot(aes(flow_cfs, suitable_acres, color = scenario)) +
  geom_line() +
  theme_minimal() +
...
knitr::kable(head(mokelumne_hrl |> 
                    mutate(FR_spw_sqm_hrl = round(FR_spw_sqm_hrl),
                           FR_juv_sqm_hrl = round(FR_juv_sqm_hrl),
                           FR_fry_sqm_hrl = round(FR_fry_sqm_hrl)) |> 
  select(`Flow (cfs)` = flow_cfs, 
         `Suitable acres (spawning)` = FR_spw_sqm_hrl,
         `Suitable acres (juvenile)` = FR_juv_sqm_hrl,
         `Suitable acres (fry)` = FR_fry_sqm_hrl)))
```

...
This document describes the process of adding instream juvenile and fry rearing, spawning, and floodplain rearing to the Upper San Joaquin reach of the [SpringRunDSM](https://github.com/Reorienting-to-Recovery/springRunDSM) (from Merced River confluence to Friant Dam). The Lower San Joaquin reach is 45.7 miles long while the Upper San Joaquin Reach is 153 miles in length. Note that the available habitat is reflective of reach-specific flows but the the flows in the DSM are from the 2018/2019 CalSim BiOp for the Lower San Joaquin reach.  

## Instream Rearing Habitat

The 1D Hec Ras model was run for for flows 100 through 4500 CFS at all nodes of the bypasses (Chowchilla, Eastside, and Mariposa) and reaches 1, 1A, 2A, and 5. A suitability criteria based on depth and velocity was applied to all nodes and flows to get a suitable area for in-channel and floodplain rearing for the Upper San Joaquin River between Merced River and Friant Dam. 
...
**Future Improvements**
Reaches used for instream and floodplain rearing can be updated to include mainstem (2B - 4B2) reaches. This dataset assumes flow primarily through the bypasses. 
...
fry_depth <- yuba_suit |> filter(life_stage == "Fry" &  suitability_metric == "Depth") |> 
  filter(suitability_index > suit_index & units_si_conv > 0) |> 
...
upper_san_joaquin_floodplain <- upper_san_joaquin_floodplain |> 
  mutate(SR_floodplain_sqm = acres_to_square_meters(SR_floodplain_acres))
...
The same 1D Hec Ras model was run for for flows 100 through 4500 CFS at reaches 1A and 1B for spawning.  A suitability based on depth and velocity was applied all nodes and flows to get a suitable area for spawning in the Upper San Joaquin River between Merced River and Friant Dam. 
...
all_results_with_suitab <- all_ras_results |> 
  ...
  filter(reach %in% c('Rch1A_Simplified', 'Rch1_Simplified')) # spawning in reach 1 (1A and 1B) per Rene
...
```{r message=FALSE, warning=FALSE}
upper_san_joaquin_rearing <- upper_san_joaquin_rearing |> 
  mutate(SR_fry_sqm = DSMhabitat::acres_to_square_meters(SR_fry_acres),
         SR_juv_sqm = DSMhabitat::acres_to_square_meters(SR_juv_acres))

upper_san_joaquin_spawning <- upper_san_joaquin_spawning |> 
  mutate(SR_spawn_sqm = DSMhabitat::acres_to_square_meters(SR_spawn_acres))
...
spawn <- expand_grid(
  ...
  mutate(
    sit_habitat = as.vector(cvpiaData::sr_spawn[31 , , ] |> 
                         DSMhabitat::square_meters_to_acres()),
    r_to_r_baseline = as.vector(DSMhabitat::sr_spawn$r_to_r_baseline['San Joaquin River', , ]) |>  
      DSMhabitat::square_meters_to_acres()
    )

spawn |> 
  transmute(watershed, date = ymd(paste(year, month, 1)), 
            sit_habitat, r_to_r_baseline) |> 
...
floodplain <- expand_grid(
  ...
  arrange(year, month, watershed) |> 
  mutate(
    sit_habitat = as.vector(cvpiaData::sr_fp[31 , , ] |> 
                         DSMhabitat::square_meters_to_acres()),
    r_to_r_baseline = as.vector(DSMhabitat::sr_fp$r_to_r_baseline['San Joaquin River', , ]) |>  
      DSMhabitat::square_meters_to_acres()
    )
...
juv <- expand_grid(
  ...
  arrange(year, month, watershed) |> 
  mutate(
    sit_habitat = as.vector(cvpiaData::sr_juv[31 , , ] |> 
                         DSMhabitat::square_meters_to_acres()),
    r_to_r_baseline = as.vector(DSMhabitat::sr_juv$biop_itp_2018_2019['San Joaquin River', , ]) |>  
      DSMhabitat::square_meters_to_acres()
    )
...
fry <- expand_grid(
  ...
  arrange(year, month, watershed) |> 
  mutate(
    sit_habitat = as.vector(cvpiaData::sr_fry[31 , , ] |> 
                         DSMhabitat::square_meters_to_acres()),
    r_to_r_baseline = as.vector(DSMhabitat::sr_fry$r_to_r_baseline['San Joaquin River', , ]) |>  
      DSMhabitat::square_meters_to_acres()
    )
